...
(function (SB) {
  ...
  function playCard(state, me, action, mods) {
    ...
    if (action.asPilot) {
      ...
      target.upgrades.push(inst);
      SB.log(state, { type: 'attached', uid: target.uid, cardId: inst.cardId, sound: 'attach' });
      // Both attach paths announce it: this is the ordinary play-from-hand one, and a
      // bearer that reacts to being upgraded must not care which way the upgrade arrived.
      SB.fireTriggers(state, 'onUpgradeAttachedSelf', target, { sourceUid: target.uid, upgradeCardId: inst.cardId });
