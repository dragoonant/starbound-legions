...
// and trait names, its printed rules text, and its words for the game vocabulary the
// theme renames (TERMS below), keyed by our internal ids, registered as a pack the
// player can switch on and off (names.js registerSource; default on).
//
...
// When the file is absent the game shows its own original names throughout.
...
//   <uq>                 the unique insignia. A card face carries no third-party
//                        symbol, so it becomes the word — the printed game's word,
//                        since this text is only ever shown under the printed names.

// ---- game vocabulary ------------------------------------------------------
// names.js `terms`: the theme's words for the power token and the unique insignia.
// These are the published game's words for the same things, registered as one more
// kind of the pack, so a generated log line, prompt, label or help paragraph reads in
// the same vocabulary as the printed text beside it — and switching the names off
// brings the theme's words back with them (THEME.md).
const TERMS = {
  force: 'the Force',           // the power, in prose: "use the Force"
  forceToken: 'a Force token',  // the token as a thing gained or spent, with its article
  forceLabel: 'Force token',    // as a label or title
  unique: 'unique',             // the insignia, as an adjective: "a unique unit"
};

const markup = v => typeof v !== 'string' ? v
  : v.replace(/<bullet>([\s\S]*?)<\/bullet>/g, (m, block) => block.split('\n')
        .map(l => (l.trim() ? '• ' + l.trim() : l)).join('\n'))
     .replace(/<\/?uq>/g, TERMS.unique);
...
const body = [
  ...
  '  var V = ' + JSON.stringify(TERMS, null, 1) + ';',
  '  SB.names.registerSource({ cards: C, traits: T, decks: D, text: X, terms: V });',
].join('\n');

const OUT = join(root, 'data', 'names-source.js');
writeFileSync(OUT,
  '// names-source.js — GENERATED by tools/gen-source-names.mjs; do not hand-edit.\n' +
  '// The published game\'s card, trait and deck names, its printed rules text and its\n' +
  '// words for the game vocabulary (V), keyed by our internal ids. Loaded by index.html\n' +
  '// only (never tests.html) and registered as a switchable pack: names.js keeps this\n' +
  '// game\'s own original names alongside. Card\n' +
